// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from gwpspider_interfaces:msg/WaterPumpCommand.idl
// generated code does not contain a copyright notice
#include "gwpspider_interfaces/msg/detail/water_pump_command__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `command`
#include "rosidl_runtime_c/string_functions.h"

bool
gwpspider_interfaces__msg__WaterPumpCommand__init(gwpspider_interfaces__msg__WaterPumpCommand * msg)
{
  if (!msg) {
    return false;
  }
  // pump
  // command
  if (!rosidl_runtime_c__String__init(&msg->command)) {
    gwpspider_interfaces__msg__WaterPumpCommand__fini(msg);
    return false;
  }
  return true;
}

void
gwpspider_interfaces__msg__WaterPumpCommand__fini(gwpspider_interfaces__msg__WaterPumpCommand * msg)
{
  if (!msg) {
    return;
  }
  // pump
  // command
  rosidl_runtime_c__String__fini(&msg->command);
}

bool
gwpspider_interfaces__msg__WaterPumpCommand__are_equal(const gwpspider_interfaces__msg__WaterPumpCommand * lhs, const gwpspider_interfaces__msg__WaterPumpCommand * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // pump
  if (lhs->pump != rhs->pump) {
    return false;
  }
  // command
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->command), &(rhs->command)))
  {
    return false;
  }
  return true;
}

bool
gwpspider_interfaces__msg__WaterPumpCommand__copy(
  const gwpspider_interfaces__msg__WaterPumpCommand * input,
  gwpspider_interfaces__msg__WaterPumpCommand * output)
{
  if (!input || !output) {
    return false;
  }
  // pump
  output->pump = input->pump;
  // command
  if (!rosidl_runtime_c__String__copy(
      &(input->command), &(output->command)))
  {
    return false;
  }
  return true;
}

gwpspider_interfaces__msg__WaterPumpCommand *
gwpspider_interfaces__msg__WaterPumpCommand__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  gwpspider_interfaces__msg__WaterPumpCommand * msg = (gwpspider_interfaces__msg__WaterPumpCommand *)allocator.allocate(sizeof(gwpspider_interfaces__msg__WaterPumpCommand), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gwpspider_interfaces__msg__WaterPumpCommand));
  bool success = gwpspider_interfaces__msg__WaterPumpCommand__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
gwpspider_interfaces__msg__WaterPumpCommand__destroy(gwpspider_interfaces__msg__WaterPumpCommand * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    gwpspider_interfaces__msg__WaterPumpCommand__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
gwpspider_interfaces__msg__WaterPumpCommand__Sequence__init(gwpspider_interfaces__msg__WaterPumpCommand__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  gwpspider_interfaces__msg__WaterPumpCommand * data = NULL;

  if (size) {
    data = (gwpspider_interfaces__msg__WaterPumpCommand *)allocator.zero_allocate(size, sizeof(gwpspider_interfaces__msg__WaterPumpCommand), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gwpspider_interfaces__msg__WaterPumpCommand__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gwpspider_interfaces__msg__WaterPumpCommand__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gwpspider_interfaces__msg__WaterPumpCommand__Sequence__fini(gwpspider_interfaces__msg__WaterPumpCommand__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gwpspider_interfaces__msg__WaterPumpCommand__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gwpspider_interfaces__msg__WaterPumpCommand__Sequence *
gwpspider_interfaces__msg__WaterPumpCommand__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  gwpspider_interfaces__msg__WaterPumpCommand__Sequence * array = (gwpspider_interfaces__msg__WaterPumpCommand__Sequence *)allocator.allocate(sizeof(gwpspider_interfaces__msg__WaterPumpCommand__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = gwpspider_interfaces__msg__WaterPumpCommand__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
gwpspider_interfaces__msg__WaterPumpCommand__Sequence__destroy(gwpspider_interfaces__msg__WaterPumpCommand__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    gwpspider_interfaces__msg__WaterPumpCommand__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
gwpspider_interfaces__msg__WaterPumpCommand__Sequence__are_equal(const gwpspider_interfaces__msg__WaterPumpCommand__Sequence * lhs, const gwpspider_interfaces__msg__WaterPumpCommand__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gwpspider_interfaces__msg__WaterPumpCommand__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gwpspider_interfaces__msg__WaterPumpCommand__Sequence__copy(
  const gwpspider_interfaces__msg__WaterPumpCommand__Sequence * input,
  gwpspider_interfaces__msg__WaterPumpCommand__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gwpspider_interfaces__msg__WaterPumpCommand);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    gwpspider_interfaces__msg__WaterPumpCommand * data =
      (gwpspider_interfaces__msg__WaterPumpCommand *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gwpspider_interfaces__msg__WaterPumpCommand__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          gwpspider_interfaces__msg__WaterPumpCommand__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gwpspider_interfaces__msg__WaterPumpCommand__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
